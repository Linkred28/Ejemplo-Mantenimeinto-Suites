...
import { Role, Ticket, TicketStatus, Urgency, MaintenanceType } from '../types';
...
const getMaintenanceIcon = (type: MaintenanceType | undefined) => {
  switch (type) {
    case 'Electricista': return <Zap className="w-3 h-3" />;
    case 'Plomero': return <Droplet className="w-3 h-3" />;
    case 'Técnico HVAC': return <Fan className="w-3 h-3" />;
    case 'Carpintero': return <Hammer className="w-3 h-3" />;
    case 'Técnico TV/Redes': return <Monitor className="w-3 h-3" />;
    case 'Cerrajero': return <Key className="w-3 h-3" />;
    ...
    >
      ...
      <div className="mb-2">
        <span className="inline-flex items-center gap-1.5 px-2 py-1 bg-slate-100 text-slate-700 text-xs rounded-md font-medium">
          {getMaintenanceIcon(ticket.maintenanceType)}
      ...
      <div className="bg-white rounded-xl shadow-2xl max-w-2xl w-full max-h-[90vh] overflow-y-auto flex flex-col">
        <div className="p-6 border-b border-slate-200 flex justify-between items-start">
          <div>
            ...
            <div className="flex items-center gap-1.5 mt-2 text-xs font-medium text-slate-600 bg-slate-100 px-2 py-1 rounded inline-flex">
              {getMaintenanceIcon(ticket.maintenanceType)}
              {ticket.maintenanceType || 'General'}
            </div>
          </div>
...
export const MaintenanceView: React.FC = () => {
  ...
  const [filterStatus, setFilterStatus] = useState<TicketStatus | 'ALL_ACTIVE' | 'ALL_CLOSED'>('ALL_ACTIVE');
  ...
  const filteredTickets = useMemo(() => {
    return tickets.filter(t => {
      ...
      if (filterStatus === 'ALL_ACTIVE' && t.status === TicketStatus.VERIFIED) return false;
      if (filterStatus === 'ALL_CLOSED' && t.status !== TicketStatus.VERIFIED) return false;
      if (filterStatus !== 'ALL_ACTIVE' && filterStatus !== 'ALL_CLOSED' && t.status !== filterStatus) return false;

      // Filter by Professional Type
      if (filterType !== 'ALL') {
        const type = t.maintenanceType || 'General';
        if (type !== filterType) return false;
      }

      // Filter by Urgency Toggle
      if (onlyUrgent && t.urgency !== Urgency.HIGH) return false;

      return true;
    });
  }, [tickets, filterType, filterStatus, onlyUrgent]);
    ...
    <div>
      <div className="flex flex-col lg:flex-row lg:justify-between lg:items-center gap-3 mb-6">
        ...
        <button
          ...
          className="inline-flex items-center gap-2 px-4 py-2 rounded-full bg-white border border-slate-200 text-slate-700 text-sm font-semibold hover:bg-slate-50 shadow-sm"
      ...
      {/* Filter Toolbar */}
      <div className="bg-white rounded-xl border border-slate-200 p-4 mb-6 shadow-sm flex flex-col md:flex-row gap-4 items-center justify-between">
        
        <div className="flex flex-col md:flex-row gap-4 w-full md:w-auto">
          {/* Status Filter */}
          <div className="relative">
            <label className="block text-xs font-semibold text-slate-400 mb-1 uppercase">Estado</label>
            <select 
              value={filterStatus} 
              onChange={(e) => setFilterStatus(e.target.value as any)}
              className="w-full md:w-48 bg-slate-50 border border-slate-200 rounded-lg px-3 py-2 text-sm font-medium text-slate-700 focus:outline-none focus:border-slate-400"
            >
              <option value="ALL_ACTIVE">Todos los Activos</option>
              <option value={TicketStatus.OPEN}>Reportado</option>
              <option value={TicketStatus.IN_PROGRESS}>En Proceso</option>
              <option value={TicketStatus.WAITING_PART}>Espera Refacción</option>
              <option value={TicketStatus.VENDOR}>Requiere Proveedor</option>
              <option value={TicketStatus.RESOLVED}>Resuelto (Sin Verificar)</option>
              <option value="ALL_CLOSED">Cerrados / Verificados</option>
            </select>
          </div>

          {/* Professional Type Filter */}
          <div className="relative">
            <label className="block text-xs font-semibold text-slate-400 mb-1 uppercase">Profesional</label>
            <select 
              value={filterType} 
            ...
            >
              ...
              <option value="Técnico HVAC">❄️ Técnico HVAC</option>
              <option value="Carpintero">🔨 Carpintero</option>
              <option value="Técnico TV/Redes">📺 Técnico TV/Redes</option>
        ...
        <div className="flex items-center gap-2 pt-5 md:pt-0">
          ...
          >
            <AlertTriangle className={`w-4 h-4 ${onlyUrgent ? 'fill-rose-700' : ''}`} />
            Solo Urgentes
          </button>
        </div>
      </div>
